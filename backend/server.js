...
const { saveEnergyReading, getAllReadings, getReadingsCount } = require('./services/dataLogger');
...
['TUYA_CLIENT_ID','TUYA_CLIENT_SECRET','TUYA_DEVICE_ID','MONGODB_URI'].forEach(k=>{
  if(!process.env[k]) {
    console.warn(`⚠ Missing env var ${k}. Set in .env or hosting platform before deployment.`);
  }
});
...
app.get('/api/live', async (req, res) => {
  try {
    ...
    // Save to database (non-blocking, won't affect response)
    saveEnergyReading(metrics.power, metrics.totalEnergy, metrics.hourlyCost).catch(err => {
      console.log('Note: Data not saved to database');
    });
...
// Get stored readings from database
...
app.listen(PORT, () => {
  ...
  console.log('  GET  /api/readings       - Get stored readings from database');
  console.log('\n' + '─'.repeat(60) + '\n');
});
